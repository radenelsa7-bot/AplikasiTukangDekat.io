...
class PaymentController extends Controller
{
    ...
    {
        $user = Auth::user();
        if ($user->role === 'ADMIN' || $user->role === 'TREASURER') return null;
        if ($order->customer_id === $user->id || $order->provider_id === $user->id) return null;
  ...
  public function getPaymentStatus($paymentId)
  {
    ...
    public function captureQris(Request $request, $paymentId)
    {
        ...
        if ($user->role === 'TREASURER' && $allowTreasurerRead) return null;
